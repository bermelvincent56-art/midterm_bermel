...
function initializeGames() {
    ...
    const colorLevel = document.getElementById('color-level');

    startColorGame.addEventListener('click', startColorMemoryGame);
    ...
    function playSequence() {
        ...
        const interval = setInterval(() => {
            const colorButton = document.querySelector(`[data-color="${colorSequence[i]}"]`);
            colorButton.classList.add('flash');
            setTimeout(() => colorButton.classList.remove('flash'), 300);
            i++;
    ...
    function gameOver() {
        ...
        startColorGame.textContent = 'Game Over - Try Again';
        setTimeout(() => {
            startColorGame.textContent = 'Start Game';
        }, 2000);
    }

    function updateColorDisplay() {
        colorScore.textContent = score;
        colorLevel.textContent = level;
    }
    ...
    const newNumberGame = document.getElementById('new-number-game');

    guessBtn.addEventListener('click', makeGuess);
    guessInput.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') makeGuess();
    });
    newNumberGame.addEventListener('click', resetNumberGame);

    function makeGuess() {
        const guess = parseInt(guessInput.value);
        if (isNaN(guess) || guess < 1 || guess > 100) {
            guessFeedback.textContent = 'Please enter a number between 1-100!';
            return;
        }
        ...
        if (guess === targetNumber) {
            guessFeedback.textContent = `🎉 Congratulations! You got it in ${attempts} attempts!`;
    ...
    function resetNumberGame() {
        ...
        guessFeedback.textContent = "I'm thinking of a number between 1-100...";
        guessFeedback.style.color = 'white';
        guessInput.value = '';
    }
    ...
    const startSpeedTest = document.getElementById('start-speed-test');
    const clickCountSpan = document.getElementById('click-count');
    const timerSpan = document.getElementById('timer');
    const speedResult = document.getElementById('speed-result');

    startSpeedTest.addEventListener('click', startClickSpeedTest);
    ...
    function endSpeedTest() {
        ...
        let rating = '';
        if (cps >= 8) rating = 'Lightning Fast! ⚡';
        else if (cps >= 6) rating = 'Very Fast! 🚀';
        else if (cps >= 4) rating = 'Good! 👍';
        else if (cps >= 2) rating = 'Not bad! 😊';
        else rating = 'Keep practicing! 💪';

        speedResult.textContent = `Result: ${clickCount} clicks (${cps} CPS) - ${rating}`;
    }
    ...
    rpsButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            playerChoiceSpan.textContent = emojis[playerChoice];
            ...
            if (result.includes('You win')) {
                playerScore++;
                rpsWinner.style.color = '#2ed573';
            } else if (result.includes('Computer wins')) {
                computerScore++;
                rpsWinner.style.color = '#ff6b6b';
            } else {
                rpsWinner.style.color = '#f39c12';
            }
